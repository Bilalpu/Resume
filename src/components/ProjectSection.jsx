...
const Url = [
  { id: '1', imageUrl: netflix, text: 'Netflix UI Clone' },
  { id: '2', imageUrl: facebo, text: 'YouTube UI Clone' },
  { id: '3', imageUrl: yoputube, text: 'Portfolio Website' },
  { id: '4', imageUrl: cherry, text: 'Facebook Login UI' },
  { id: '5', imageUrl: prot, text: 'Design Your Home' },
  { id: '6', imageUrl: fff, text: 'Charity Clone Website' },
];
  ...
  return (
    <div>
      ...
      <div className="allProject">
        <div className="six-image">
          {Url.map((item) => (
            <ProjectItem key={item?.id} item={item} delay={delay} />
          ))}
        </div>
      </div>
    </div>
...
function ProjectItem({ item, delay }) {
  const [currentText, setCurrentText] = useState('');
  const [currentIndex, setCurrentIndex] = useState(0);
  ...
  useEffect(() => {
    ...
    if (hovered && currentIndex < item.text.length) {
      ...
      // Reset the text and index after hover ends
      setCurrentText('');
      setCurrentIndex(0);
    }
